...
function TopBar({ previewOpen, onTogglePreview, dockApi }) {
  ...
  return (
    <header className="relative flex h-11 shrink-0 items-center gap-3 border-b bg-card px-3">
      ...
      {/* Global search — not in Merge Studio, where there's no file tree
          or command set for it to search; it's absolutely centered over
          the header (not a flex sibling of the left/right clusters), so
          leaving it out here doesn't unbalance anything else in the row —
          the flow stepper below lives on the canvas, in its own
          independently centered box, entirely unaffected either way. */}
      {!inMergeStudio && (
